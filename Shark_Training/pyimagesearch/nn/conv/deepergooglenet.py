...
class DeeperGoogLeNet:
    ...
    def conv_module(x, K, kX, kY, stride, chanDim, padding="same", reg=0.0005,
        ...
        # Initialize the CONV, BN, and RELU layer names as None
        (convName, bnName, actName) = (None, None, None)

        # Prepend a name prefix, if required
        if name is not None:
            convName = name + "_conv"
            bnName = name + "_bn"
            actName = name + "_act"
    ...
    def inception_module(x, num1x1, num3x3Reduce, num3x3, num5x5Reduce,
        ...
        # Define the first branch: 1x1 convolutions
        first = DeeperGoogLeNet.conv_module(x, num1x1, 1, 1, (1, 1),
                                            chanDim, reg=reg,
                                            name=stage + "_first")

        # Define the second branch: 1x1 and 3x3 convolutions
        ...
        # Define the third branch: 1x1 and 5x5 convolutions
        ...
        # Define the fourth branch: 3x3 POOL projection
    ...
    def build(width, height, depth, classes, reg=0.0005):
        ...
        # Define the model input, followed by a sequence of layers:
