...
if args["validfrac"] > 0:
    ...
    print(validCntSer)
outDirs = [os.path.join(args["outdir"], "snapshots"),
...
if args["validfrac"] > 0:
    validTab["imageName"] = validTab["imageName"].apply(lambda x:
                                                os.path.join(absDataPath, x))
trainTab.loc[trainTab.label == "NEG",
             ["x1","y1","x2","y2", "label"]] = np.nan
if args["validfrac"] > 0:
    validTab.loc[validTab.label == "NEG",
                 ["x1","y1","x2","y2", "label"]] = np.nan
outAnnCSV = os.path.join(args["outdir"], "ann_train.csv")
print("[INFO] creating training annotation file in the output directory:")
print("\t[{}]".format(outAnnCSV))
trainTab.to_csv(outAnnCSV, header=False, index=False, float_format="%.0f")
if args["validfrac"] > 0:
    outAnnCSV = os.path.join(args["outdir"], "ann_valid.csv")
    print("[INFO] creating a validation annotation" +
          "file in the output directory:")
    print("\t[{}]".format(outAnnCSV))
    validTab.to_csv(outAnnCSV, header=False, index=False, float_format="%.0f")
uniqueLabels.remove("NEG") if "NEG" in uniqueLabels else None
