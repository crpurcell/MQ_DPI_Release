package com.berapnopod.sharktest.tracking;
...
import com.berapnopod.sharktest.tflite.Detector;

import com.berapnopod.sharktest.env.BorderedText;
import com.berapnopod.sharktest.env.ImageUtils;
...
public class MultiBoxTracker {
    ...
    public synchronized void trackResults(
            final List<Detector.Result> results, final byte[] frame, final long timestamp) {
    ...
    public synchronized void draw(final Canvas canvas) {
        ...
        numShark = 0;
        for (final TrackedRecognition recognition : trackedObjects) {
            if(recognition.detectionConfidence < 0.60f) continue;
            numShark++;
        ...
        //*borderedText.drawText(canvas, 30, canvas.getHeight() - 50, "SHARK: " + numShark); *//
    }
    ...
    private void processResults(
            final long timestamp, final List<Detector.Result> results, final byte[] originalFrame) {
        ...
        for (final Detector.Result result : results) {
            ...
            // Count each shark above the threshhold
            if(result.getConfidence() > 0.60f) {
                numShark++;
            }
            ...
            rectsToTrack.add(new Pair<Float, Detector.Result>(result.getConfidence(), result));
        }
    ...
    private void handleDetection(
            final byte[] frameCopy, final long timestamp, final Pair<Float, Detector.Result> potential) {
